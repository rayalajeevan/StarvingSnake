...
class AboutActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val ani_slide=android.view.animation.AnimationUtils.loadAnimation(applicationContext,R.animator.slide_up)
        for(i in 0..frame.childCount-1){
            val x=frame.getChildAt(i)
            x.startAnimation(ani_slide)
        }
        about.startAnimation(ani_slide)
        frame.startAnimation(ani_slide)
